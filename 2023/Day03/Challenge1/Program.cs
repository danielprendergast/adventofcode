...
//string[] strInputArray = File.ReadAllLines("testinput.txt");
...
int iCurrentRow = 0;
var listIndexes = new List<Tuple<int, int>>();
int iTotalValue = 0;
...
foreach (string strInputLine in strInputArray)
{
    foreach (string strSymbol in strSymbolArray)
    {
        for (int i = 0; i < strInputLine.Length; i++)
        {
            ...
            {
                listIndexes.Add(new Tuple<int, int>(iCurrentRow, (i)));
            }
        }
    }
    iCurrentRow++;
...
foreach (string strInputLine in strInputArray)
{
    ...
    foreach (Match match in matchedNumbers)
    {
        ...
        for (int i = 0; i < iNumberLength; i++)
        {
            if (!bNearSymbol)
            {
                if (listIndexes.Contains(new Tuple<int, int>(iCurrentRow, iPosition - 1 + i)) || listIndexes.Contains(new Tuple<int, int>(iCurrentRow, iPosition + 1 + i)) || listIndexes.Contains(new Tuple<int, int>(iCurrentRow + 1, iPosition + i)) || listIndexes.Contains(new Tuple<int, int>(iCurrentRow + 1, iPosition + 1 + i)) || listIndexes.Contains(new Tuple<int, int>(iCurrentRow + 1, iPosition - 1 + i)) || listIndexes.Contains(new Tuple<int, int>(iCurrentRow - 1, iPosition + i)) || listIndexes.Contains(new Tuple<int, int>(iCurrentRow - 1, iPosition + 1 + i)) || listIndexes.Contains(new Tuple<int, int>(iCurrentRow - 1, iPosition - 1 + i)))
                {
        ...
        if (bNearSymbol)
        {
            iTotalValue += int.Parse(match.ToString());
        }
